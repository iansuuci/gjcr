#!/usr/bin/env python3
"""
OpenAI ABLATION Experiment: Token-Limited Reasoning
...
Usage:
    python openai_aime_abalation.py --percent 10   # 10% of avg tokens (default)
    python openai_aime_abalation.py --percent 70   # 70% of avg tokens

Average across all conditions: 6,985 tokens
"""
...
# ========================= Token Configuration =========================
# Average across all conditions: (8914 + 8976 + 9136 + 363 + 7536) / 5 = 6985
AVG_TOKENS = 6985
...
class ModelRunner:
    def __init__(self, api_key: Optional[str] = None, 
        ...
        print(f"Initializing OpenAI API client (ABLATION MODE)")
        print(f"  Model: {model}")
    ...
    def generate(self, prompt: str, use_reasoning: bool = True, max_tokens: int = 1000) -> Tuple[str, Dict]:
        ...
        try:
            ...
            if hasattr(response, 'usage') and response.usage:
                token_usage["prompt_tokens"] = response.usage.input_tokens or 0
                token_usage["completion_tokens"] = response.usage.output_tokens or 0
                token_usage["total_tokens"] = (response.usage.input_tokens or 0) + (response.usage.output_tokens or 0)
            
            # Extract response text
            if hasattr(response, 'output_text') and response.output_text:
                return response.output_text.strip(), token_usage
            ...
            if hasattr(response, 'output') and response.output:
                ...
                if text_parts:
                    return "\n".join(text_parts).strip(), token_usage
...
def main():
    parser = argparse.ArgumentParser(description="OpenAI Ablation Test with configurable token limit")
    ...
    print("=" * 80)
    print(f"ABLATION TEST: OpenAI with {percent}% Token Limit")
    print("=" * 80)
    ...
    output_file = os.path.join(os.path.dirname(__file__), "..", "abalation_results", f"openai_abalation_{percent}pct_results.json")
