#!/usr/bin/env python3
"""
DeepSeek ABLATION Experiment: Token-Limited Reasoning
...
Usage:
    python deepseek_aime_abalation.py --percent 10   # 10% of avg tokens (default)
    python deepseek_aime_abalation.py --percent 70   # 70% of avg tokens

Per-condition averages from normal run:
- only_code: 8,088 tokens
- only_comments: 7,870 tokens
- both: 8,338 tokens
- nothing: 0 tokens (no data)
- cot: 8,960 tokens

Average across all conditions: 6,651 tokens
"""
...
# ========================= Token Configuration =========================
# From DeepSeek results: (8088 + 7870 + 8338 + 0 + 8960) / 5 = 6651
AVG_TOKENS = 6651
...
class ModelRunner:
    def __init__(self, api_key: Optional[str] = None, 
        ...
        print(f"Initializing DeepSeek API client (ABLATION MODE)")
...
def main():
    parser = argparse.ArgumentParser(description="DeepSeek Ablation Test with configurable token limit")
    ...
    print("=" * 80)
    print(f"ABLATION TEST: DeepSeek with {percent}% Token Limit")
    print("=" * 80)
    ...
    output_file = os.path.join(os.path.dirname(__file__), "..", "abalation_results", f"deepseek_abalation_{percent}pct_results.json")
