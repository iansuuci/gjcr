#!/usr/bin/env python3
"""
Grok ABLATION Experiment: Token-Limited Reasoning
...
Usage:
    python grok_aime_abalation.py --percent 10              # 10% of avg tokens (default)
    python grok_aime_abalation.py --percent 70              # 70% of avg tokens
    python grok_aime_abalation.py --percent 70 --parallel   # 70% with parallel execution (faster)

Average across all conditions: 5,664 tokens
"""
...
# ========================= Token Configuration =========================
# Average across all conditions: (7002 + 6674 + 5626 + 1370 + 7650) / 5 = 5664
AVG_TOKENS = 5664
...
class ModelRunner:
    def __init__(self, api_key: Optional[str] = None, 
        ...
        print(f"Initializing Grok API client (ABLATION MODE)")
...
def run_single_condition(runner: ModelRunner, q: str, condition: str, token_limit: int, percent: int) -> Dict:
...
def evaluate_problem(runner: ModelRunner, prob: Dict, index: int, token_limit: int, percent: int, parallel: bool = False) -> Dict:
    ...
    if parallel:
        # Parallel execution - run all 5 conditions concurrently
        print(f"  Running {len(conditions)} conditions in parallel...")
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {
                executor.submit(run_single_condition, runner, q, cond, token_limit, percent): cond 
...
def main():
    parser = argparse.ArgumentParser(description="Grok Ablation Test with configurable token limit")
    ...
    print("=" * 80)
    print(f"ABLATION TEST: Grok with {percent}% Token Limit")
    print("=" * 80)
    ...
    # Save results with percentage in filename
    output_file = os.path.join(os.path.dirname(__file__), "..", "abalation_results", f"grok_abalation_{percent}pct_results.json")
