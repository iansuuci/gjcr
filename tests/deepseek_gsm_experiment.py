#!/usr/bin/env python3
"""
DeepSeek Experiment: Can models reason in code? (GSM8K Version)

This experiment measures the effect of code vs comments vs both vs nothing
on model performance using the GSM8K test dataset and DeepSeek API.
...
try:
    ...
    print(f"Loaded {len(GSM8K_TEST_PROBLEMS)} GSM8K test problems")
...
def extract_final_answer_from_text(text: str) -> Optional[int]:
    ...
    patterns = [
        r"\\boxed\{(-?[\d,]+)\}",  # LaTeX boxed answer (highest priority)
        r"\*\*Final Answer[:\s]*(-?[\d,]+)\*\*",
        r"Final Answer[:\s]*(-?[\d,]+)",
...
class ModelRunner:
    def __init__(self, api_key: Optional[str] = None, 
                 ...
                 non_reasoning_model: str = "deepseek-chat"):
        """
        Initialize DeepSeek API client.
        
        Args:
            api_key: DeepSeek API key. If None, will try to get from DEEPSEEK_API_KEY env var.
            reasoning_model: Model name for reasoning mode (default: "deepseek-reasoner")
            non_reasoning_model: Model name for non-reasoning mode (default: "deepseek-chat")
        """
        if api_key is None:
        ...
        print(f"Initializing DeepSeek API client")
        print(f"  Reasoning model: {reasoning_model}")
        print(f"  Non-reasoning model: {non_reasoning_model}")
        # DeepSeek API uses OpenAI-compatible SDK with custom base URL
        self.client = OpenAI(
            api_key=api_key,
...
def main():
    ...
    non_reasoning_model = "deepseek-chat"
    print("=" * 80)
    print("Can models reason in code? - DeepSeek Experiment (GSM8K)")
    print("=" * 80)
    print(f"Reasoning Model: {reasoning_model}")
    print(f"Non-Reasoning Model: {non_reasoning_model}")
    print(f"Dataset: GSM8K Test ({len(GSM8K_TEST_PROBLEMS)} problems)")
    ...
    output_file = "deepseek_gsm_experiment_results.json"
